...
#       self.ldap_dcu.simple_bind_s('', '')
    ...
    def check_userfree(self, uid):
        ...
        if res:
            raise RBFatalError("Username '%s' is already taken by %s account (%s)" %
        ...
        if res:
            self.rberror(RBWarningError("Username '%s' is reserved (%s)" %
                                        (uid, res[0][1]['description'][0].decode())))
    ...
    def check_user_byid(self, user_id):
        ...
        user database."""
        if not self.ldap.search_s(rbconfig.ldap_accounts_tree, ldap.SCOPE_ONELEVEL, 'id=%s' %
                                  user_id):
    ...
    def check_group_byid(self, gid):
        ...
        group database."""
        if not self.ldap.search_s(rbconfig.ldap_group_tree, ldap.SCOPE_ONELEVEL, 'gidNumber=%s' %
                                  gid):
...
#       res = self.ldap.search_s(rbconfig.ldap_accounts_tree, ldap.SCOPE_ONELEVEL, 'uid=%s' %
#                                usr.uid, ('objectClass',))
#           if res:
...
#                   raise RBFatalError("User '%s' does not exist" % uid)
    ...
    def get_userinfo_new(self, usr, override=0):
        ...
        if usr.id is not None:
            ...
            try:
                ...
                raise RBFatalError("Id '%s' is already registered to %s (%s)" % (usr.id, tmpusr.uid,
                                                                                 tmpusr.cn))
    ...
    def get_userdefaults_new(cls, usr):
        ...
        if usr.newbie is None:
            usr.newbie = 1
        if (usr.yearsPaid is None and (usr.usertype in rbconfig.usertypes_paying) and
...
#                               pass
    ...
    def get_student_byid(self, usr, override=0):
        ...
        res = self.ldap_dcu.search_s(rbconfig.ldap_dcu_students_tree, ldap.SCOPE_SUBTREE,
    ...
    def get_alumni_byid(self, usr, override=0):
        ...
        res = self.ldap_dcu.search_s(rbconfig.ldap_dcu_alumni_tree, ldap.SCOPE_SUBTREE,
                                     'cn=%s' % usr.id)
        if res:
            self.set_user_dcu(usr, res[0], override)
    ...
    def get_staff_byid(self, usr, override=0):
        ...
        #
        res = self.ldap_dcu.search_s(rbconfig.ldap_dcu_staff_tree, ldap.SCOPE_SUBTREE,
    ...
    def get_dummyid(self, usr):
        ...
        res = self.ldap.search_s(rbconfig.ldap_accounts_tree, ldap.SCOPE_ONELEVEL,
    ...
    def get_group_byid(self, gid):
        ...
        Raise RBFatalError if given id does not belong to a group in
        group database."""
        res = self.ldap.search_s(rbconfig.ldap_group_tree, ldap.SCOPE_ONELEVEL, 'gidNumber=%s' %
                                 gid)
        if res:
            return res[0][1]['cn'][0]
        else:
    ...
    def check_email(self, usr):
        ...
        if (usr.usertype in ('member', 'staff') and not
            ...
            self.rberror(RBWarningError("%s users should have a DCU email address" %
                                        (usr.usertype)))
    ...
    def set_passwd(self, usr):
        ...
        self.wrapper(self.ldap.modify_s, self.uid2dn(usr.uid),
                     ((ldap.MOD_REPLACE, 'userPassword', usr.userPassword),))

    def set_shell(self, usr):
        """Set shell for given user."""

        self.wrapper(self.ldap.modify_s, self.uid2dn(usr.uid),
                     ((ldap.MOD_REPLACE, 'loginShell', usr.loginShell),))
    ...
    def list_reserved(self):
        ...
        res = self.ldap.search_s(rbconfig.ldap_reserved_tree, ldap.SCOPE_ONELEVEL,
                                 'objectClass=reserved', ('uid',))
        return [data['uid'][0] for dn, data in res]
    ...
    def dict_reserved_desc(self):
        ...
        description."""
        res = self.ldap.search_s(rbconfig.ldap_reserved_tree, ldap.SCOPE_ONELEVEL,
                                 'objectClass=reserved', ('uid', 'description'))
        tmp = {}
        for data in res:
    ...
    def search_dcu(self, where, var):
        ...
        raise RBFatalError("NOT IMLEMENTED YET")
        var = '%%%s%%' % var
        self.cur.execute(('SELECT u.username, u.usertype, s.id, s.name, s.course, s.year, s.email '
                          'FROM students s LEFT JOIN users u USING (id) WHERE ') + where + ' %s',
                         (var,))
        res = self.cur.fetchall()
        self.cur.execute(('SELECT u.username, u.usertype, s.id, s.name, s.email '
                          'FROM staff s LEFT JOIN users u USING (id) WHERE ') + where + '%s',
                         (var,))
        return [(username, usertype, id, name, None, None, email)
                for username, usertype, id, name, email
                in self.cur.fetchall()] + res
    ...
    def stats(self):
        ...
        ordered_usertypes = list(rbconfig.usertypes_list) + [i for i in rbconfig.usertypes
                                                             if i not in rbconfig.usertypes_list]
        ...
        print("Quorum (rounded-up square root of above):", math.ceil(math.sqrt(total_paid)))
        print("'Active' users (paid and non-paying signed-in users):",
        ...
        print("%d of %d newbies signed-in (%d%%)\n" %
              (category_totals['signed_newbie'], category_totals['newbie'],
               100.0 * category_totals['signed_newbie'] / category_totals['newbie']))
    ...
    def crypt(cls, password):
        ...
        return crypt.crypt(password, saltchars[random.randrange(len(saltchars))] +
                           saltchars[random.randrange(len(saltchars))])

    @classmethod
    def userPassword(cls, password):
        """Return string suitable for LDAP userPassword attribute based on given plaintext
        password."""
    ...
    def uidNumber_getnext(self):
        ...
        while 1:
            try:
                ...
                retries += 1
                if retries == 20:
                    raise RBFatalError(('Could not lock uidNumber.txt file after 20 attempts.'
                                        'Please try again!'))
                time.sleep(0.5)
            else:
                break
